...
df = pd.read_csv('ch_hist.csv', skiprows=1)
    ...
    for jj in range(0, len(str_split)):
        ...
        if (split2[1] == 'Exact' and split2[4] == 'max' and split2[5] == 'CPC'):
            ...
            for kk in range(1, num_kw+1):
                ...
                kw_indx1 = str_split[jj + kk].index('[')
                kw_indx2 = str_split[jj + kk].index(']')
                kw = str_split[jj + kk][kw_indx1 + 1 : kw_indx2]
                tmp_split = re.sub('\[[^]]+\]', 'hahaha', str_split[jj +kk])
                split3 = re.split('\$', tmp_split)
